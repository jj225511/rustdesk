...
pub fn handle_msg_clipboard(mut cb: Clipboard) {
    ...
    if let Ok(bytes) = multi_clips.write_to_bytes() {
        let _ = scrap::android::ffi::call_main_service_update_clipboard(&bytes);
    }
}
...
#[cfg(target_os = "android")]
pub fn get_clipboards_msg() -> Option<Message> {
    let mut clipboards = scrap::android::ffi::get_clipboards()?;
    let mut msg = Message::new();
