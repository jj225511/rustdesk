...
#[inline]
fn remove_custom_client_staging_dir(staging_dir: &PathBuf) -> ResultType<bool> {
...
pub fn prepare_custom_client_update() -> ResultType<()> {
    ...
    if custom_client_staging_dir.exists() {
        ...
        if custom_txt_path.is_file() {
            ...
            if let Err(e) = fs::remove_file(&local_custom_file_path) {
                log::error!(
                    "Failed to remove temporary custom.txt file {:?}: {}",
                    local_custom_file_path,
                    e
                );
                drop(clear_staging_on_exit);
                // Make sure that "custom.txt" will not be copied to the installation directory.
                std::process::exit(1);
            }
