...
pub fn main_load_recent_peers() {
    if !config::APP_DIR.read().unwrap().is_empty() {
        ...
        if load_two_times {
            ...
            log::info!(
                "REMOVE ME ============================= low tow times: first load time: {:?}",
                now.elapsed()
            );
            ...
            log::info!(
                "REMOVE ME ============================= low tow times: second load time: {:?}",
                now.elapsed()
            );
        } else {
            let now = std::time::Instant::now();
            ...
            log::info!(
                "REMOVE ME ============================= low one time: load time: {:?}",
                now.elapsed()
            );
        }
        push_to_flutter(serde_json::ser::to_string(&all_peers).unwrap_or("".to_owned()));
...
pub fn main_get_recent_peers(get_all: bool) -> String {
    if !config::APP_DIR.read().unwrap().is_empty() {
        ...
        log::info!(
            "REMOVE ME ============================= get recent peers, load_count {}, time: {:?}",
            load_count,
            now.elapsed()
        );
        return xx;
