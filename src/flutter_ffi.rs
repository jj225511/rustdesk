...
pub fn main_load_recent_peers() {
    if !config::APP_DIR.read().unwrap().is_empty() {
        ...
        if load_two_times {
            ...
            log::info!("REMOVE ME ============================= low tow times: first load time: {:?}, count: {}", now.elapsed(), all_peers.len());
            ...
            log::info!("REMOVE ME ============================= low tow times: second load time: {:?}, count: {}", now.elapsed(), all_peers.len());
        } else {
            let now = std::time::Instant::now();
            let _ = load_recent_peers(&vec_id_modified_time_path, true, &mut all_peers, 0);
            log::info!("REMOVE ME ============================= low one time: load time: {:?}, count: {}", now.elapsed(), all_peers.len());
        }
        push_to_flutter(serde_json::ser::to_string(&all_peers).unwrap_or("".to_owned()));
    }
}
...
pub fn main_get_recent_peers(get_all: bool) -> String {
    if !config::APP_DIR.read().unwrap().is_empty() {
        ...
        let batch_loading_count = PeerConfig::get_search_loading_batch_count();
