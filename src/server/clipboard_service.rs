...
#[cfg(target_os = "android")]
use scrap::android::ffi::{call_clipboard_manager_enable_clipboard, EnableClipboard};
use std::{
    io,
...
#[cfg(target_os = "android")]
fn is_clipboard_enabled() -> EnableClipboard {
    ...
    if keyboard_enabled && clip_enabled {
...
#[cfg(target_os = "android")]
fn run(sp: EmptyExtraFieldService) -> ResultType<()> {
    let _res = call_clipboard_manager_enable_clipboard(is_clipboard_enabled());
    while sp.ok() {
        if let Some(msg) = crate::clipboard::get_clipboards_msg() {
            sp.send(msg);
        }
        std::thread::sleep(Duration::from_millis(INTERVAL));
    }
    let _res = call_clipboard_manager_enable_clipboard(EnableClipboard::ServerDisable);
    Ok(())
}
