...
impl TerminalSession {
    ...
    fn stop(&mut self) {
        ...
        if let Some(input_tx) = self.input_tx.take() {
            ...
            // This required on Windows and Linux.
            input_tx.send(b"\r\n".to_vec()).ok();
            drop(input_tx);
        }
...
impl TerminalServiceProxy {
    ...
    fn handle_open(
        ...
        let writer_thread = thread::spawn(move || {
            ...
            // 1. Windows needs at least one carriage return, so `drop()` can work properly.
            //    No idea why the reader can't read the buf after `input_tx.send(b"\r\n".to_vec()).ok();` if no carriage return is sent before.
            // 2. To refresh the terminal interface on the controlling side. (workaround for the content is blank on connect)
