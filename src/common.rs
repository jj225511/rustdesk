...
pub async fn post_request(url: String, body: String, header: &str) -> ResultType<String> {
    let proxy_conf = Config::get_socks();
    let tls_url = get_url_for_tls(&url, &proxy_conf);
    let response = match post_request_inner(&url, tls_url, body.clone(), header, None).await? {
        Some(resp) => resp,
        None => {
            match post_request_inner(&url, tls_url, body, header, Some(TlsType::Rustls)).await? {
                Some(resp) => resp,
...
async fn post_request_inner(
    ...
    if tls_type.is_none() {
        tls_type = get_cached_tls_type(tls_url);
    }
...
pub async fn http_request_sync(
    ...
    let tls_url = get_url_for_tls(&url, &proxy_conf);
    let response =
        match get_http_response_async(&url, tls_url, &method, body.clone(), &header, None).await? {
            Some(resp) => resp,
            None => {
                match get_http_response_async(
                    &url,
                    tls_url,
                    &method,
                    body,
