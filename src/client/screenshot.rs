...
impl Screenshot {
    pub fn set_take_screenshot(&mut self, display: Option<i32>) {
    ...
    pub fn handle_screenshot(&mut self, action: String) -> String {
        let Some(rgba) = self.rgba.take() else {
            return "No cached screenshot".to_owned();
        };

        std::thread::spawn(move || {
            if let Err(e) = Self::handle_screenshot_(rgba, action) {
                // to-do: push error message to the ui
            }
        });
        "".to_owned()
    }

    fn handle_screenshot_(mut rgba: ImageRgb, action: String) -> ResultType<()> {
        #[cfg(any(target_os = "macos", not(feature = "flutter")))]
        if rgba.raw.len() >= rgba.h * rgba.w * 4 {
            let stride = rgba.raw.len() / rgba.h;
            for y in 0..rgba.h {
                for x in 0..rgba.w {
                    let i = stride * y + 4 * x;
                    let x = rgba.raw[i];
                    rgba.raw[i] = rgba.raw[i + 2];
                    rgba.raw[i + 2] = x;
                }
            }
        }
        match ScreenshotAction::from(&action as &str) {
            ScreenshotAction::SaveAs(p) => {
                // fix-me: this function maybe very slow, and we need to move it to a stanalone thread.
...
#[cfg(not(feature = "flutter"))]
pub fn set_take_screenshot(display: Option<i32>) {
