...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/opt/rustdesk')
    ...
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/opt/rustdesk/')
    system2(
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/rustdesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/opt/rustdesk/')
    system2(
        'cp ../res/rustdesk.service tmpdeb/usr/share/rustdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/rustdesk.png')
...
def main():
    ...
    '''.format(pa, version))
                ...
                system2('mkdir -p tmpdeb/opt/rustdesk')
                system2('mv tmpdeb/usr/bin/rustdesk tmpdeb/opt/rustdesk/')
                system2('cp libsciter-gtk.so tmpdeb/opt/rustdesk/')
                md5_file('usr/share/rustdesk/files/systemd/rustdesk.service')
                md5_file('etc/rustdesk/startwm.sh')
                md5_file('etc/X11/rustdesk/xorg.conf')
                md5_file('etc/pam.d/rustdesk')
                md5_file('opt/rustdesk/libsciter-gtk.so')
