...
fn try_fill_positions(
    ...
    for (i, sd) in shared_displays.iter_mut().enumerate() {
        if let crate::Display::WAYLAND(d) = sd {
            ...
            hbb_common::log::info!("============================== Trying to disambiguate capturable at position {:?} with size {:?}, matched count: {}",
                capturable.position, capturable.physical_size, match_count);
            if match_count == 0 {
                warn!(
...
// Move curosr to the left upper corner and compare the pixel data
// to determine if the display matches.
#[derive(Clone)]
struct RoiRecord {
    ts: u128,
    x: i32,
...
lazy_static! {
    static ref ROI_RECORDS: Mutex<Vec<RoiRecord>> = Mutex::new(Vec::new());
}
fn compare_left_up_cornor(x: i32, y: i32, w: usize, d1: &[u8], d2: &[u8]) -> bool {
    if w == 0 {
        return false;
    }
    if d1.len() != d2.len() {
        ...
        for x in 0..roi_w {
            ...
            if a != b {
                ...
                if diff_px >= threshold {
                    // debug!("================================= diff_px: {}, threshold: {}", diff_px, threshold);
                    // return true;
                }
            }
        }
    ...
    debug!(
        "================================= diff_px: {}, threshold: {}, total_px: {}",
        diff_px, threshold, total_px
    );

    // Store ROI buffers for later centralized saving.
    let mut roi_buf1: Vec<u8> = Vec::with_capacity(roi_w * roi_h * bpp);
    let mut roi_buf2: Vec<u8> = Vec::with_capacity(roi_w * roi_h * bpp);
    for y in 0..roi_h {
        let row_start = y * stride;
        let row1 = &d1[row_start..row_start + roi_w * bpp];
        let row2 = &d2[row_start..row_start + roi_w * bpp];
        roi_buf1.extend_from_slice(row1);
        roi_buf2.extend_from_slice(row2);
    }
    ROI_RECORDS.lock().unwrap().push(RoiRecord {
...
fn fill_multi_matched_positions_cursor(
        ...
        match (
            ...
            (Some(crate::Display::WAYLAND(d)), Some(pw_stream), Some(pw_stream_with_cursor)) => {
                ...
                for (i, wd) in displays.displays.iter().enumerate() {
                    ...
                    {
                        hbb_common::log::info!(
                            "================================== Attempting to disambiguate capturable with size {:?} to display at ({}, {}).",
                            d.0.physical_size, wd.x, wd.y);
                        for i in 0..3 {
                        ...
                        match rec.capture(CAPTURE_TIMEOUT_MS) {
                            Ok(PixelProvider::BGR0(_, _, data2)) if is_bgr => {
                                if compare_left_up_cornor(wd.x, wd.y, w, &first_buf, data2) {
                                    d.0.position = (wd.x, wd.y);
                                    pw_stream.position = (wd.x, wd.y);
                                    mached_indices.push(i);
                                    debug!(
    ...
    // Centralized saving of recorded ROIs (cursor variant)
    {
        let mut records = ROI_RECORDS.lock().unwrap();
        if !records.is_empty() {
            let dir = "/home/username/Desktop";
            let _ = std::fs::create_dir_all(dir);
            for rec in records.iter_mut() {
                for (id, buf_src) in [(1u8, &rec.buf1), (2u8, &rec.buf2)] {
                    let mut buf_rgba = buf_src.clone(); // BGRA -> RGBA
                    for px in buf_rgba.chunks_exact_mut(4) {
                        px.swap(0, 2);
                    }
                    let path = format!("{}/{}_{}_{}_{}_{}.png", dir, rec.ts, rec.x, rec.y, id, rec.diff_px);
                    if let Err(e) = image::save_buffer(
                        ...
                        debug!("Failed to save ROI PNG {}: {}", path, e);
                    } else {
                        debug!("Saved ROI PNG: {}", path);
                    }
                }
            }
            records.clear();
