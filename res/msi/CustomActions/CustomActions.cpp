...
void RecursiveDelete(LPCWSTR path, MSIHANDLE hInstall)
{
    ...
    StringCchPrintfW(searchPath, MAX_PATH, L"%s\\*", path);
    ...
    if (hFind == INVALID_HANDLE_VALUE)
    {
        ...
        WcaLog(LOGMSG_STANDARD, "RecursiveDelete: Could not find first file in '%ls'. It might be empty or missing. Error: %lu", path, GetLastError());
        return;
    }
    ...
    do
    {
        ...
        StringCchPrintfW(fullPath, MAX_PATH, L"%s\\%s", path, findData.cFileName);
        ...
        if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
        {
            ...
            {
                WcaLog(LOGMSG_STANDARD, "RecursiveDelete: Failed to remove directory. Error: %lu", GetLastError());
            }
        }
        else
        {
            ...
            {
                WcaLog(LOGMSG_STANDARD, "RecursiveDelete: Failed to delete file. Error: %lu",GetLastError());
            }
        }
